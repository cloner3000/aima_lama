...
class Standar1AuditorController extends Controller
{
    public function index(){
      $standar1 = Standar1Auditor::where("id_prodi", 1)->get();
    	$data=array();
    ...
    public function update(Request $request){
      if($request->skor1_1_a_old != $request->skor1_1_a){
        Standar1Auditor::where("kode", "1.1.a")->update(["kategori"=>$request->skor1_1_a, "data"=>"[".$request->skor1_1_a."]", "skor"=>$request->skor1_1_a]);
      }

      if($request->skor1_1_b_old != $request->skor1_1_b){
        Standar1Auditor::where("kode", "1.1.b")->update(["kategori"=>$request->skor1_1_b, "data"=>"[".$request->skor1_1_b."]", "skor"=>$request->skor1_1_b]);
      }

      if($request->skor1_2_old != $request->skor1_2){
        Standar1Auditor::where("kode", "1.2")->update(["kategori"=>$request->skor1_2, "data"=>"[".$request->skor1_2."]", "skor"=>$request->skor1_2]);
      }
