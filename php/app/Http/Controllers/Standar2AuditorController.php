...
class Standar2AuditorController extends Controller
{
  public function index(){
    $standar2 = Standar2Auditor::where("id_prodi", 1)->get();
    $data=array();
    foreach ($standar2 as $data_standar2) {
      $data[$data_standar2->kode]=$data_standar2->kategori;
    }
    $standar="Standar 2";
    return view("auditor/standar2.index", compact('data', 'standar'));
  }
  ...
  public function update(Request $request){
    if($request->skor2_1_old != $request->skor2_1){
      Standar2Auditor::where("kode", "2.1")->update(["kategori"=>$request->skor2_1, "data"=>"[".$request->skor2_1."]", "skor"=>$request->skor2_1]);
    }

    if($request->skor2_2_old != $request->skor2_2){
      Standar2Auditor::where("kode", "2.2")->update(["kategori"=>$request->skor2_2, "data"=>"[".$request->skor2_2."]", "skor"=>$request->skor2_2]);
    }

    if($request->skor2_6_old != $request->skor2_6){
      Standar2Auditor::where("kode", "2.6")->update(["kategori"=>$request->skor2_6, "data"=>"[".$request->skor2_6."]", "skor"=>$request->skor2_6]);
    }
